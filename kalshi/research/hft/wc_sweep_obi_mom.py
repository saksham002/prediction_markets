"""WC sweep: OBI gated by short-horizon MOMENTUM sign (agree_om_<hl>) under a
real $1000 deployed-capital budget. Tests whether confirming obi with very-short
mom (HL 1s/5s/10s/30s) beats plain obi. In-sample = first 4 WC games, out = last 4.

Usage: wc_sweep_obi_mom.py --shard I --num-shards N   |   wc_sweep_obi_mom.py --collect
"""
import argparse
import json
import sys
...
RESULTS = Path("/data/user_data/saksham3/kalshi_hft/sims/wc_sweep_obi_mom")
BEST_CFG = Path("/data/user_data/saksham3/kalshi_hft/studies/wc_obi_mom_best_config.json")
BUDGET = 1000.0

# gated obi keeps obi's magnitude, so reuse the obi |alpha| percentiles {50,75,90,95,99}
_PCT = json.load(open(Path("/data/user_data/saksham3/kalshi_hft/studies/wc_thresholds.json")))
THRS = sorted(set(_PCT["obi"].values()))
MOM_HLS = ["1s", "5s", "10s", "30s"]
SIZES = [10, 50, 200]
CAPS = [10, 50, 200, 1000, 5000]

COMBOS = [(f"om_{hl}", f"agree_om_{hl}", thr, s, cap)
          for hl in MOM_HLS for thr in THRS for s in SIZES for cap in CAPS if s <= cap]
...
def run_shard(shard, n):
    ...
    for idx, (label, alpha, thr, s, cap) in enumerate(COMBOS):
        ...
        with open(f, "w") as fh:
            json.dump(res, fh)
        print(f"{label:>7} t{thr:<7g} s{s:<4} c{cap:<5} in_net={res['in']['net']:+8.1f} "
              f"out_net={res['out']['net']:+8.1f}", flush = True)


def collect():
    rows = [json.load(open(p)) for p in sorted(RESULTS.glob("wc_om_*.json"))]
    print(f"{len(rows)}/{len(COMBOS)} combos done  (BUDGET=${BUDGET:g})\n")
    print(f"{'hl':>7} {'thr':>8} {'size':>5} {'cap':>5} {'IN_net':>9} {'OUT_net':>9} "
          f"{'IN_rn':>9} {'OUT_rn':>9} {'in_fills':>8}")
    for r in sorted(rows, key = lambda r: -r["in"]["net"]):
        print(f"{r['label']:>7} {r['thr']:>8g} {r['size']:>5g} {r['cap']:>5g} "
              f"{r['in']['net']:>+9.1f} {r['out']['net']:>+9.1f} "
              f"{r['in']['realized_net']:>+9.1f} {r['out']['realized_net']:>+9.1f} {r['in']['fills']:>8}")
    print("\n=== per-HL: best IN-sample config -> its OUT-sample ===")
    for hl in MOM_HLS:
        sub = [r for r in rows if r["label"] == f"om_{hl}"]
        if sub:
            b = max(sub, key = lambda r: r["in"]["net"])
            print(f"  om_{hl:>3}: best-in thr={b['thr']:g} s={b['size']} cap={b['cap']} -> "
